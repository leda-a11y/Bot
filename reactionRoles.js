...
module.exports = async (client) => {
  try {
    // Promeni sa stvarnim ID tvog servera
    const guild = await client.guilds.fetch("1260323486321217536"); 

    // Fetch kanala po imenu ili po ID (bolje po ID)
    const channel = guild.channels.cache.find(c => c.name === "﹝🔶﹞ʀᴏʟᴏᴠɪ") 
                  || await guild.channels.fetch("1476700778109341827"); 
